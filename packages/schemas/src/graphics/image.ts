 ...
 * permanent Map key whose byte length matched the image itself. A
 * fingerprint based on length + first/last 16 characters is sufficient
 * to distinguish different images while keeping the key under 60 chars.
 * Collisions are effectively impossible for realistic image payloads: a
 * match requires identical byte length, identical first 16 chars, and
 * identical last 16 chars simultaneously.
 */
const getCacheKey = (schema: Schema, input: string) =>
  `${schema.type}:${input.length}:${input.slice(0, 16)}:${input.slice(-16)}`;
